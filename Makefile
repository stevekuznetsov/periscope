...
sub-deployment:
    ...
    oc create configmap sub-config --from-file=sub.json=config/sub.json -o yaml --dry-run | oc apply -f -
    oc apply -f openshift/sub-deployment.yaml
.PHONY: sub-deployment

poll-deployment:
    oc create configmap poll-config --from-file=poll.json=config/poll.json -o yaml --dry-run | oc apply -f -
    oc apply -f openshift/poll-deployment.yaml
.PHONY: sub-deployment

psql-deployment:
    oc create configmap psql-config --from-file=postgresql.json=config/postgresql.json -o yaml --dry-run | oc apply -f -
